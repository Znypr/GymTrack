...
fun TimeOfDayHeatmap(
    ...
    val emptyCellColor = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.08f)

    fun getCellColor(value: Int): Color {
        if (value == 0) return emptyCellColor
        val fraction = value.toFloat() / maxCount
        val alpha = when {
            fraction < 0.25f -> 0.28f
            fraction < 0.5f -> 0.48f
            fraction < 0.75f -> 0.72f
            else -> 1.0f
        }
        return theme.primary.copy(alpha = alpha)
    }
    ...
    Column(modifier.fillMaxWidth()) {
        ...
        Canvas(modifier = Modifier.fillMaxWidth().height(240.dp)) {
            val layout = ChartLayout(leftPad = 48f, rightPad = 32f, topPad = 8f, bottomPad = 42f)
            ...
            for (h in 0..24 step 6) {
                ...
                val x = (x0 + h * cellW).coerceIn(0f, size.width - labelWidth)
                drawContext.canvas.nativeCanvas.drawText(label, x, y0 + chartH + 30f, labelPaint)
            }
        }
    }
}
