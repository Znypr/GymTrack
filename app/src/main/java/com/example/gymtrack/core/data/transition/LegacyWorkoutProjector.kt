...
internal class LegacyWorkoutProjector(
    ...
    fun project(
        ...
        if (note.text.isNotBlank() && blocks.isEmpty()) {
            reviewReasons += "no exercise blocks found"
        }
        ...
        blocks.forEachIndexed { blockPosition, block ->
            ...
            val parsedNames = parsedSets
                .map { CanonicalKeys.normalize(it.exerciseName) }
                .filter(String::isNotEmpty)
                .distinct()
            if (parsedNames.size > 1) {
                reviewReasons += "exercise ${blockPosition + 1} has inconsistent set names"
            }

            val fallbackName = cleanHeader(block.header.text)
            val exerciseName = parsedSets.firstOrNull()?.exerciseName
            ...
            workoutExercises += CanonicalWorkoutExerciseEntity(
                ...
                legacyAbsoluteTimeText = block.header.elapsedText.takeIf(String::isNotBlank)
                    ?: parsedSets.firstOrNull()?.absoluteTime,
            )
        ...
        sourceLines.forEach { line ->
            when {
                ...
                line.text.firstOrNull()?.isWhitespace() == true -> {
                    if (current == null) {
                        reviewReasons += "set row without exercise header"
                    } else {
                        current?.setLines?.add(line)
                    }
                }
                else -> {
    ...
    private fun cleanHeader(raw: String): String = raw
        .trim()
        .replace(Regex("\\s*\\(\\s*\\d+['’]\\d{2}(?:''|\")?\\s*\\)\\s*$"), "")
        .trim()

    private fun parseElapsedSeconds(raw: String): Int? {
        val match = Regex("^\\s*(\\d+)['’](\\d{2})(?:''|\")?\\s*$").matchEntire(raw)
            ?: return null
    ...
    private data class SourceLine(
        ...
        val flag: ExerciseFlag,
    )

    private data class ExerciseBlock(
        val header: SourceLine,
        val setLines: List<SourceLine>,
    ) {
    ...
    private data class MutableExerciseBlock(
        val header: SourceLine,
        val setLines: MutableList<SourceLine> = mutableListOf(),
    ) {
